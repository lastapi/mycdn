...
<form class="search-form" id="search-form" action="https://www.google.com/search" method="GET" target="_blank">
  ...
  function load_content(data) {
    ...
    for (let data_backlink of (data?.data_backlink || [])) {
      ...
      let link_backlink = window.location.protocol + "//" + getRandomItems(MyDomain,1) + '.' + getRandomItems(MyApiDomain, 1) + "/" + "?" + data_backlink?.path + "=" + id_backlink;// + "&update=" + new Date().getTime();
      dom_related_posts += `
